...
Sometimes a random effects model is required with a large number of smooth curves, which really are the random effects. That is, we might have a random smooth curve per subject, these can  be set up to be efficiently computed in `gamm` and `gamm4`
